class Thread:
    def __init__(
        ...
        self.internal_thread_diameter = (
            self.nominal_thread_diameter - 2 * self.height_coil
        )  # d1
        self.midle_thread_diameter = (
            self.nominal_thread_diameter - 2 * 3 * self.full_profile_height / 8
        )  # d2
        self.bolt_active_area = (
            3.14
            * (self.internal_thread_diameter**2 - self.bolt_hole_diameter**2)
            / 4
        )  # Sb
        self.nut_active_area = (
            3.14
            * (self.nut_minimum_diameter**2 - self.nominal_thread_diameter**2)
            / 4
        )  # Sn

        # Calculation stress permis
        self.bolt_tension_stress_permis = (
            self.bolt_yield_strength / self.k_industry
        )  # [G]рб
        self.nut_tension_stress_permis = (
            self.nut_yield_strength / self.k_industry
        )  # [G]рг
        ...
        self.bolt_crush_stress = self.axial_force / (
            3.14 * self.midle_thread_diameter * self.height_coil * self.work_threads
        )  # [G]смб
        ...
        self.k_bolt_tension = round(
            (self.bolt_tension_stress_permis / self.bolt_tension_stress), 1
        )  # [G]рб
        self.k_nut_tension = round(
            (self.nut_tension_stress_permis / self.nut_tension_stress), 1
        )  # [G]рг
        self.k_bolt_crush = round(
            (self.bolt_crush_stress_permis / self.bolt_crush_stress), 1
        )  # [G]смб
        self.k_nut_crush = round(
            (self.nut_crush_stress_permis / self.nut_crush_stress), 1
        )  # [G]смг
        self.k_bolt_shear = round(
            (self.bolt_shear_stress_permis / self.bolt_shear_stress), 1
        )  # [т]смб
        self.k_nut_shear = round(
            (self.nut_shear_stress_permis / self.nut_shear_stress), 1
        )  # [т]смг
