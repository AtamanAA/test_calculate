...
class PipeHighPressure(models.Model):
    ...
    def get_by_id(pipe_id):
        return (
            PipeHighPressure.objects.get(id=pipe_id)
            if PipeHighPressure.objects.filter(id=pipe_id)
            else None
        )
    ...
    def get_by_user_order_by_created(user_id):
        return (
            PipeHighPressure.objects.filter(user=user_id)
            .order_by(
                "created_at",
            )
            .reverse()
...
class ThreadConnection(models.Model):
    ...
    def __str__(self):
        return (
            f"Thread {self.name} {self.nominal_thread_diameter} x {self.thread_pitch}"
        )
    ...
    def __thread(self):
        object = Thread(
            self.axial_force,
            self.bolt_yield_strength,
            self.nut_yield_strength,
            self.nominal_thread_diameter,
            self.thread_pitch,
            self.nut_active_height,
            self.nut_minimum_diameter,
    ...
    def get_by_id(thread_id):
        return (
            ThreadConnection.objects.get(id=thread_id)
            if ThreadConnection.objects.filter(id=thread_id)
            else None
        )
    ...
    def get_by_user_order_by_created(user_id):
        return (
            ThreadConnection.objects.filter(user=user_id)
            .order_by(
                "created_at",
            )
            .reverse()
    ...
    def create(
        ...
        max_bolt_hole_diameter = (
            thread.nominal_thread_diameter - 2 * thread.thread_pitch
        )
    ...
    def update(
        self,
        axial_force=None,
        bolt_yield_strength=None,
        nut_yield_strength=None,
        nominal_thread_diameter=None,
        thread_pitch=None,
        nut_active_height=None,
        nut_minimum_diameter=None,
