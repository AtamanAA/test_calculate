...
from .services import pipe_thickness
...
class PipeThicknessTest(TestCase):
    def test_thickness_k_1(self):
        expected = 0.82        
        actual = pipe_thickness(yield_strength=300, test_pressure=10, min_outside_diameter=50,
                                k_welding=1, k_industry=1, k_cycle=1)
        self.assertEqual(actual, expected)
    
    def test_thickness_k_industry(self):
        expected = 1.613        
        actual = pipe_thickness(yield_strength=300, test_pressure=10, min_outside_diameter=50,
                                k_welding=1, k_industry=2, k_cycle=1)
        self.assertEqual(actual, expected)

    def test_thickness_k_weldind(self):
        expected = 1.02        
        actual = pipe_thickness(yield_strength=300, test_pressure=10, min_outside_diameter=50,
                                k_welding=0.8, k_industry=1, k_cycle=1)
        self.assertEqual(actual, expected)

    def test_thickness_k_cycle(self):
        expected = 1.613       
        actual = pipe_thickness(yield_strength=300, test_pressure=10, min_outside_diameter=50,
                                k_welding=1, k_industry=1, k_cycle=2)
        self.assertEqual(actual, expected)

    def test_thickness_zero_k(self):
        with self.assertRaises(ZeroDivisionError):
            pipe_thickness(yield_strength=300, test_pressure=-10, min_outside_diameter=50,
                            k_welding=0, k_industry=0, k_cycle=0)
