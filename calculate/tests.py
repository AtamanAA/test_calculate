...
class PipeThicknessTest(TestCase):
    def test_thickness_k_1(self):
        expected = 0.82
        actual = PipeHighPressure.calculate(
            yield_strength=300,
            test_pressure=10,
            min_outside_diameter=50,
            k_industry=1,
            k_cycle=1,
            k_welding=1,
        ).thickness
...
class ThreadTest(TestCase):
    thread = Thread(
        axial_force=10,
        bolt_yield_strength=300,
        nut_yield_strength=300,
        nominal_thread_diameter=10,
        thread_pitch=1.5,
        nut_active_height=10,
        nut_minimum_diameter=40,
