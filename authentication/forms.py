...
class RegisterUserForm(UserCreationForm):
    username = forms.CharField(
        label="Ім'я",
        max_length=15,
        error_messages={"max_length": "Введіть коротше ім'я"},
    )
    email = forms.EmailField(
        label="E-mail",
        max_length=40,
        error_messages={"invalid": "Введіть коректну адресу пошти!"},
    )
    password1 = forms.CharField(
        label="Пароль", max_length=20, widget=forms.PasswordInput
    )
    password2 = forms.CharField(
        label="Повторити пароль", max_length=20, widget=forms.PasswordInput
    )
...
class PasswordsChangeForm(PasswordChangeForm):
    ...
    new_password2 = forms.CharField(
        label="Повторити новий пароль", widget=forms.PasswordInput
    )
